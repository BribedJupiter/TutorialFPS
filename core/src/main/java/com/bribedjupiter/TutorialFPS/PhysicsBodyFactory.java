...
public class PhysicsBodyFactory implements Disposable {
    ...
    public PhysicsBody createBody(ModelInstance collisionInstance, CollisionShapeType shapeType, float mass, boolean isStatic) {
        ...
        if (isStatic) {
            geom.setCategoryBits(CATEGORY_STATIC); // static category
            geom.setCollideBits(0); // which categories will it colide with?
        ...
        // Create debug model matching collision geom shape
        modelBuilder.begin();
